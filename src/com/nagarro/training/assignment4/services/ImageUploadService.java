...
public class ImageUploadService {
	public Boolean uploadUserImage(HttpServletRequest request)
		...
		try {
			...
			new ImageService().validateFormData(userid, formData); // Throws NewCustomException if
			...
			uploaded = new ImageDaoImpl().uploadUserImage(userImg);

			new UserService().updateTotalImageSize(userid, imageDataInBytes.length, -1);
